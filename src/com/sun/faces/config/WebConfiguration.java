...
public class WebConfiguration {
	...
	private static final Pattern ALLOWABLE_BOOLEANS = Pattern.compile(
	...
	public void doPostBringupActions() {
		...
		// enabled.
		boolean enabled = this
				.isOptionEnabled(BooleanWebContextInitParameter.EnableHttpMethodRestrictionPhaseListener);
		if (enabled) {
	...
	private void processJndiEntries(String contextName) {
		 ...
		 * InitialContext(); } catch (NamingException ne) { if
		 ...
		 * .equals(WebEnvironmentEntry.ClientStateSavingPassword)) { if (LOGGER
		 * .isLoggable(loggingLevel)) { LOGGER.log(loggingLevel,
	...
	private boolean canProcessJndiEntries() {
		...
		/*
		 * incompatible with Google App Engine try {
		 * Util.getCurrentLoader(this).loadClass("javax.naming.InitialContext");
		 * } catch (Exception e) { if (LOGGER.isLoggable(Level.FINE)) {
		 * LOGGER.fine(
	...
	public enum WebContextInitParameter {
		...
		ManagedBeanFactoryDecorator(
				...
				"com.sun.faces.faceletCache", ""), FaceletsProcessingFileExtensionProcessAs(
				"", "");
	...
	public enum BooleanWebContextInitParameter {
		...
		DisplayConfiguration("com.sun.faces.displayConfiguration", false), ValidateFacesConfigFiles(
				...
				false), EnableHttpMethodRestrictionPhaseListener(
				...
				"com.sun.faces.cacheResourceModificationTimestamp", false), EnableAgressiveSessionDirtying(
				"com.sun.faces.enableAgressiveSessionDirtying", false), EnableMissingResourceLibraryDetection(
